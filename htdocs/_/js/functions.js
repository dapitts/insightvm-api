...
(function($)
{
	...
	{		
		...
		*/
		...
		$(function() 
		{
			$(document).on('click', '.show-hide-password .input-group-addon', function () 
			{
				var _password_input = $(this).prev();
				var _password_eye   = $(this).children();
				var _pw_attr_type   = _password_input.attr('type');
				
				switch (_pw_attr_type)
				{
					case 'password':
						_password_input.attr('type', 'text');
						_password_eye.removeClass('fa-eye-slash').addClass('fa-eye');
						break;
					case 'text':
						_password_input.attr('type', 'password');
						_password_eye.removeClass('fa-eye').addClass('fa-eye-slash');
						break;
				}
			});
		});
		...
		$(function() 
		{
			$(document).on('change', '#use_proxy_server', function () 
			{
				var _proxy_settings = $('#proxy-settings');

				if ($(this).is(':checked'))
				{
					_proxy_settings.show();
				}
				else
				{
					_proxy_settings.hide();
				}
			});
		});
